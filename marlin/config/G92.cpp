    ...
    case 0:
      LOOP_LOGICAL_AXES(i) {
        if (parser.seenval(AXIS_CHAR(i))) {
          ...
          // Big Plotter: G92 *is* the homing procedure (no endstop
          // switches; G28 is never used). Mark the axis homed/trusted so
          // software endstops apply from here on. Outside the NEAR_ZERO
          // guard above so "G92 X0 Y0" arms them even when the position
          // is already 0 (e.g. set-home pressed right after power-on).
          if (TERN1(HAS_EXTRUDERS, i != E_AXIS)) {
            set_axis_trusted((AxisEnum)i);
            set_axis_homed((AxisEnum)i);
          }
        }
      }
      break;
